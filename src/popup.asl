...
(df view-model [] -> (Map Str Str)
  ...
  (let [(m0 (map-empty))
        ...
        (m5 (map-set m4 "pending-action" "None"))]
    m5))
...
(df render-popup [(vm (Map Str Str))] -> Str
  ...
  (let [(title (option-or (map-get vm "title") "ASL Agent Plugin"))
        ...
        (pending (option-or (map-get vm "pending-action") "None"))]
    (str "<!DOCTYPE html>\n<html lang=\"en\">\n<head>\n  <meta charset=\"UTF-8\" />\n"
         "  <title>" title "</title>\n"
         ...
         "    <button id=\"btn-approve\">Approve Dangerous Action</button>\n"
         "  </div>\n"
         "  <div id=\"pending\" class=\"pending\">Pending Action: " pending "</div>\n"
